using DomainDrivenDesign.ValueObjects;

namespace DomainDrivenDesign.Tests
{
    [TestClass]
    public class UnitTest1
    {
        [TestMethod]
        public void Email()
        {
            Assert.IsTrue(new Email("domain.com").Invalid);
            Assert.IsTrue(new Email(string.Empty).Invalid);
            Assert.IsTrue(new Email("test@domain.com").Valid);
        }

        [TestMethod]
        public void Order()
        {
            var customer = CustomerFactory.Create("djgkg", "djdjd", "ldgsgdgs@test.com");

            var product = ProductFactory.Create("1 wre", 100000);

            var item = OrderItemFactory.Create(product, 1);

            var order = OrderFactory.Create(customer);

            order.AddItem(item);

            var discount = new DiscountService().Calculate(order.Total, DiscountType.Large);

            order.ApplyDiscount(discount);

            Assert.AreEqual(50000, order.Total.Value);
        }
    }
}